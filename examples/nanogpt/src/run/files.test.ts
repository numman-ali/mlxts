...
describe("run files", () => {
  test("path helpers and round-trip readers/writers behave canonically", async () => {
    await withTempRunDirectory("nanogpt-runs-root", async (repoRoot) => {
      ...
      const packageRoot = join(repoRoot, "examples", "nanogpt");
      ...
      expect(packageRootFromRunDir(directory)).toBe(
        resolve(directory, "..", "..", "examples", "nanogpt"),
      );
      ...
      writeRunStatus(directory, {
        ...
        bestCheckpoint: join(checkpointsDir(directory), "gpt-tiny-best"),
        bestCheckpointStep: 6,
        ...
        earlyStopPatience: 8,
        earlyStopMinDelta: 0.02,
        earlyStopConsecutiveBadEvals: 3,
    ...
    return withTempRunDirectory("nanogpt-run-invalid", (directory) => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          ...
          createdAt: "2026-04-04T00:00:00.000Z",
          repoRoot: "/repo",
          packageRoot: "/repo/examples/nanogpt",
          checkpointDir: checkpointsDir(directory),
