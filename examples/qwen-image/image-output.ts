...
export type QwenImageBmpWriteResult = {
  path: string;
  width: number;
  height: number;
  bytes: number;
...
function assertSingleRgbImage(image: MxArray): { height: number; width: number; channels: number } {
  ...
  if (
    ...
    throw new Error(
      `Qwen-Image output expects one NHWC RGB image, got [${image.shape.join(",")}].`,
    );
  }
...
/** Write one generated Qwen-Image tensor as an uncompressed BMP artifact. */
export function writeQwenImageBmp(image: MxArray, outputPath: string): QwenImageBmpWriteResult {
  if (!outputPath.toLowerCase().endsWith(".bmp")) {
    throw new Error("Qwen-Image output path must end with .bmp.");
  }
