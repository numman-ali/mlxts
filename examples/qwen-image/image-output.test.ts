...
import { qwenImageToBmpBytes, writeQwenImageBmp } from "./image-output";

function temporaryDirectory(): string {
  return mkdtempSync(join(import.meta.dir, ".tmp-bmp-"));
}

describe("Qwen-Image output", () => {
  test("encodes a single NHWC RGB tensor as a 24-bit BMP", () => {
    ...
    const bytes = qwenImageToBmpBytes(image);
  ...
  test("writes BMP files and creates parent directories", () => {
    ...
    try {
      ...
      const result = writeQwenImageBmp(image, outputPath);
  ...
  test("rejects batch and output format shapes outside the proof contract", () => {
    ...
    expect(() => qwenImageToBmpBytes(batched)).toThrow("one NHWC RGB image");
    expect(() => qwenImageToBmpBytes(grayscale)).toThrow("one NHWC RGB image");
    expect(() => writeQwenImageBmp(grayscale, "sample.png")).toThrow(".bmp");
  });
});
