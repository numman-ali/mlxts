...
import { fluxImageToBmpBytes, writeFluxBmp } from "./image-output";

function temporaryDirectory(): string {
  return mkdtempSync(join(import.meta.dir, ".tmp-bmp-"));
}

describe("FLUX image output", () => {
  test("encodes a single NHWC RGB tensor as a 24-bit BMP", () => {
    ...
    const bytes = fluxImageToBmpBytes(image);
  ...
  test("writes BMP files and creates parent directories", () => {
    ...
    try {
      ...
      const result = writeFluxBmp(image, outputPath);
  ...
  test("rejects batch and output format shapes outside the proof contract", () => {
    ...
    expect(() => fluxImageToBmpBytes(batched)).toThrow("one NHWC RGB image");
    expect(() => fluxImageToBmpBytes(grayscale)).toThrow("one NHWC RGB image");
    expect(() => writeFluxBmp(grayscale, "sample.png")).toThrow(".bmp");
  });
});
