...
export type FluxBmpWriteResult = {
  path: string;
  width: number;
  height: number;
  bytes: number;
...
function assertSingleRgbImage(image: MxArray): { height: number; width: number; channels: number } {
  ...
  if (
    ...
    throw new Error(
      `FLUX image output expects one NHWC RGB image, got [${image.shape.join(",")}].`,
    );
  }
...
/** Write one generated FLUX image tensor as an uncompressed BMP artifact. */
export function writeFluxBmp(image: MxArray, outputPath: string): FluxBmpWriteResult {
  if (!outputPath.toLowerCase().endsWith(".bmp")) {
    throw new Error("FLUX image output path must end with .bmp.");
  }
