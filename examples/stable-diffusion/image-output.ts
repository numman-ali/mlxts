...
export type StableDiffusionBmpWriteResult = {
  path: string;
  width: number;
  height: number;
  bytes: number;
...
function assertSingleRgbImage(image: MxArray): { height: number; width: number; channels: number } {
  ...
  if (
    ...
    throw new Error(
      `Stable Diffusion image output expects one NHWC RGB image, got [${image.shape.join(",")}].`,
    );
  }
...
/** Write one generated Stable Diffusion image tensor as an uncompressed BMP artifact. */
export function writeStableDiffusionBmp(
  image: MxArray,
  outputPath: string,
): StableDiffusionBmpWriteResult {
  if (!outputPath.toLowerCase().endsWith(".bmp")) {
    throw new Error("Stable Diffusion image output path must end with .bmp.");
  }
