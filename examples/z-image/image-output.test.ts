...
import { writeZImageBmp, zImageToBmpBytes } from "./image-output";

function temporaryDirectory(): string {
  return mkdtempSync(join(import.meta.dir, ".tmp-bmp-"));
}

describe("Z-Image output", () => {
  ...
  test("rejects batch and output format shapes outside the proof contract", () => {
    ...
    expect(() => zImageToBmpBytes(batched)).toThrow("one NHWC RGB image");
    expect(() => zImageToBmpBytes(grayscale)).toThrow("one NHWC RGB image");
    expect(() => writeZImageBmp(grayscale, "sample.png")).toThrow(".bmp");
  });
});
