...
export type Ltx2AudioAutoencoderWeightLoadOptions = {
  /** Throw when the checkpoint contains unsupported tensor names. */
  strictUnexpectedWeights?: boolean;
};

/** Assignment summary returned after loading LTX-2 audio VAE decoder weights. */
export type Ltx2AudioAutoencoderWeightLoadResult = {
...
function nextNode(current: unknown, segment: string, path: string): unknown {
  if (Array.isArray(current)) {
    ...
    if (!Number.isInteger(index) || index < 0 || index >= current.length) {
      throw new Error(
        `assignLtx2AudioAutoencoderWeightPath: "${path}" segment "${segment}" is invalid.`,
      );
    }
  ...
  if (typeof current !== "object" || current === null) {
    throw new Error(
      `assignLtx2AudioAutoencoderWeightPath: "${path}" cannot descend through a non-object.`,
    );
  }
  return Reflect.get(current, segment);
}

function assignLtx2AudioAutoencoderWeightPath(root: object, path: string, tensor: MxArray): void {
  ...
  for (let index = 0; index < segments.length - 1; index += 1) {
    ...
    if (segment === undefined) {
      throw new Error(
        `assignLtx2AudioAutoencoderWeightPath: "${path}" contains an undefined segment.`,
      );
    }
  ...
  if (typeof current !== "object" || current === null || Array.isArray(current)) {
    throw new Error(
      `assignLtx2AudioAutoencoderWeightPath: "${path}" does not point to an object property.`,
    );
  }
  const leafKey = segments[segments.length - 1];
  if (leafKey === undefined) {
    throw new Error(`assignLtx2AudioAutoencoderWeightPath: "${path}" is missing a leaf segment.`);
  }
  const existing = Reflect.get(current, leafKey);
  if (!(existing instanceof MxArray)) {
    throw new Error(
      `assignLtx2AudioAutoencoderWeightPath: "${path}" does not point to an MxArray parameter.`,
    );
  }
...
function ltx2AudioAutoencoderComponent(
  manifest: DiffusionSnapshotManifest,
): DiffusionSnapshotComponent {
  const component = manifest.components.find(
    (candidate) => candidate.name === "audio_vae" && candidate.enabled,
  );
  if (component === undefined) {
    throw new DiffusionConfigError("LTX-2 snapshot manifest is missing an enabled audio VAE.");
  }
  return component;
}
...
function camelCaseDecoderWeightPath(checkpointName: string): string | null {
  ...
  return checkpointName
    ...
    .replaceAll("block_1", "block1")
    .replaceAll("block_2", "block2")
    .replaceAll("attn_1", "attn1")
    .replaceAll("proj_out", "projOut")
    .replaceAll("nin_shortcut", "ninShortcut");
}

/** Map a Diffusers LTX-2 audio VAE tensor name onto the package decoder tree. */
export function ltx2AudioAutoencoderWeightPath(checkpointName: string): string | null {
...
/** Transform Diffusers LTX-2 audio VAE tensors into package-owned parameter layout. */
export function transformLtx2AudioAutoencoderWeight(_weightPath: string, tensor: MxArray): MxArray {
...
function throwIfUnexpectedWeights(
  unexpectedWeights: readonly string[],
  options: Ltx2AudioAutoencoderWeightLoadOptions,
): void {
  if (unexpectedWeights.length === 0 || options.strictUnexpectedWeights !== true) {
    return;
  }
  throw new Error(
    `loadLtx2AudioAutoencoderWeights: checkpoint contained unexpected unmapped weights: ${[
      ...unexpectedWeights,
    ]
...
function consumeLatentStat(
  ...
  model: Ltx2AudioAutoencoderKL,
  path: string,
  tensor: MxArray,
  assignedPaths: Set<string>,
): void {
  try {
    const values = statValues(path, tensor, model.latentStatSize);
...
function assignWeightTensor(
  ...
  try {
    const transformed = transformLtx2AudioAutoencoderWeight(path, tensor);
    if (transformed !== tensor) {
      tensor.free();
    }
    assignedTensor = transformed;
    assignLtx2AudioAutoencoderWeightPath(model, path, assignedTensor);
    assignedPaths.add(path);
    assignedTensor = null;
  } finally {
    assignedTensor?.free();
...
function consumeCheckpointTensor(
  ...
  const path = ltx2AudioAutoencoderWeightPath(checkpointName);
...
/** Load Diffusers safetensors weights into an LTX-2 audio decoder-only VAE module. */
export async function loadLtx2AudioAutoencoderWeights(
  model: Ltx2AudioAutoencoderKL,
  component: DiffusionSnapshotComponent,
  options: Ltx2AudioAutoencoderWeightLoadOptions = {},
): Promise<Ltx2AudioAutoencoderWeightLoadResult> {
...
/** Construct and load the LTX-2 audio decoder-only VAE from a snapshot manifest. */
export async function loadLtx2AudioAutoencoderFromSnapshot(
  manifest: DiffusionSnapshotManifest,
  options: Ltx2AudioAutoencoderWeightLoadOptions = {},
): Promise<Ltx2AudioAutoencoderKL> {
  const configs = await loadLtxComponentConfigs(manifest);
  if (configs.pipelineKind !== "ltx2") {
    throw new DiffusionConfigError("loadLtx2AudioAutoencoderFromSnapshot requires LTX2Pipeline.");
  }
  const model = new Ltx2AudioAutoencoderKL(configs.audioVae);
  try {
    await loadLtx2AudioAutoencoderWeights(model, ltx2AudioAutoencoderComponent(manifest), options);
    model.eval();
