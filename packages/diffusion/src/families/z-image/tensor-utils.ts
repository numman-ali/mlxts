...
export function assertFeature2d(x: MxArray, owner: string): { length: number; channels: number } {
  const [length, channels] = x.shape;
  if (x.shape.length !== 2 || length === undefined || channels === undefined) {
    throw new Error(`${owner}: expected rank-2 feature input, got ${formatShape(x.shape)}.`);
  }
...
export function assertIds2d(ids: MxArray, owner: string): { length: number } {
  const [length, axes] = ids.shape;
  if (ids.shape.length !== 2 || length === undefined || axes !== 3) {
    throw new Error(`${owner}: expected ids shape [length, 3], got ${formatShape(ids.shape)}.`);
  }
  return { length };
}
