...
function nextNode(current: unknown, segment: string, path: string): unknown {
  if (Array.isArray(current)) {
    ...
    if (!Number.isInteger(index) || index < 0 || index >= current.length) {
      throw new Error(
        `assignQwenImageWeightPath: "${path}" segment "${segment}" is not a valid array index.`,
      );
    }
  ...
  if (typeof current !== "object" || current === null) {
    throw new Error(
      `assignQwenImageWeightPath: "${path}" cannot descend through a non-object segment.`,
    );
  }
...
function assignQwenImageWeightPath(root: object, path: string, tensor: MxArray): void {
  if (path.trim() === "") {
    throw new Error("assignQwenImageWeightPath: path must not be empty.");
  }
  ...
  for (let index = 0; index < segments.length - 1; index += 1) {
    ...
    if (segment === undefined) {
      throw new Error(`assignQwenImageWeightPath: "${path}" contains an undefined segment.`);
    }
    current = nextNode(current, segment, path);
  }

  if (typeof current !== "object" || current === null || Array.isArray(current)) {
    throw new Error(`assignQwenImageWeightPath: "${path}" does not point to an object property.`);
  }

  const leafKey = segments[segments.length - 1];
  if (leafKey === undefined) {
    throw new Error(`assignQwenImageWeightPath: "${path}" is missing a leaf segment.`);
  }

  const existing = Reflect.get(current, leafKey);
  if (!(existing instanceof MxArray)) {
    throw new Error(`assignQwenImageWeightPath: "${path}" does not point to an MxArray parameter.`);
  }
...
function transformedAutoencoderWeight(
  ...
  const transformed = transformQwenImageAutoencoderWeight(checkpointName, weightPath, tensor);
...
export async function loadQwenImageAutoencoderWeights(
...
): Promise<QwenImageAutoencoderWeightLoadResult> {
  ...
  for (const shardPath of shardPaths) {
    for await (const { name, tensor } of iterateSafetensors(shardPath)) {
      const path = qwenImageAutoencoderWeightPath(name);
...
export async function loadQwenImageAutoencoderFromSnapshot(
  manifest: DiffusionSnapshotManifest,
  options: QwenImageAutoencoderWeightLoadOptions = {},
): Promise<QwenImageAutoencoderKL> {
