...
Stable Diffusion 3 / 3.5, plus LTX-Video and LTX-2 manifest recognition. The
...
| LTX-Video / LTX-2 | Diffusers `LTXPipeline`, `LTXConditionPipeline`, and `LTX2Pipeline` manifest recognition, including video VAE, LTX-2 audio VAE, text connectors, transformer, vocoder, and FlowMatch scheduler metadata | Future `examples/ltx-video` | Not yet run | Snapshot entry point only; runtime generation remains future work |
...
LTX runtime generation, image-to-image, inpainting, ControlNet, Omni/SigLIP,
