...
| Qwen-Image / Qwen-Image-2512 | Qwen-Image transformer, 3D causal VAE, FlowMatch, true-CFG denoising, weight loading, plus Qwen-Image Edit / Edit Plus snapshot recognition, reference-latent prep, multi-segment RoPE, and `zero_cond_t` transformer support | `examples/qwen-image` | Official `Qwen/Qwen-Image-2512` bounded proof passed | Forward Qwen image-generation path; edit snapshots and VAE image encode are recognized, but edit denoising concat/slice remains follow-up |
...
SD3 authenticated checkpoint proof, Qwen-Image edit denoising, FLUX.2
