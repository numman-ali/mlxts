...
| LTX-Video / LTX-2 | Diffusers `LTXPipeline`, `LTXConditionPipeline`, and `LTX2Pipeline` manifest recognition plus typed component config parsing for video VAE, LTX-2 audio VAE, text connectors, transformer, vocoder, and FlowMatch scheduler metadata | Future `examples/ltx-video` | Not yet run | Snapshot/config entry point only; runtime generation remains future work |
