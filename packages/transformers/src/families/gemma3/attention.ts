...
function ropeOffsetForCache(cache: DecoderCache | undefined): number | MxArray {
  ...
  throw new Error("Gemma3Attention.forward: unsupported batch cache implementation.");
}
...
function retainAttentionMask(
  ...
  return createCausalMask(sequenceLength, totalKeyLength, visiblePastLength, dtype, windowSize);
}
...
export class Gemma3Attention extends Module {
  ...
  run(x: MxArray, cache?: DecoderCache, attentionMask?: AttentionMask): MxArray {
    ...
    try {
      using rotatedQueries = this.rope.forward(normalizedQueries, ropeOffset);
      using rotatedKeys = this.rope.forward(normalizedKeys, ropeOffset);
