...
export type SupervisedRunSupervisorOptions = {
  trainerCommand: (spec: RunSpec) => string[];
};
...
export async function runSupervisedSupervisor(
  options: SupervisedRunSupervisorOptions,
  argv = process.argv,
): Promise<void> {
  ...
  const trainer = Bun.spawn(options.trainerCommand(spec), {
