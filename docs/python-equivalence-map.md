...
| **diffusers** | Stable Diffusion, FLUX, Qwen-Image, Z-Image, LTX, and broader media generation pipelines | @mlxts/diffusion | 10 | Partial / Hardening | Stable Diffusion / SDXL package surfaces exist: local-or-Hub Diffusers snapshot resolution, scheduler/config loading, manifest inspection, VAE/UNet construction/loading, sampling, pipeline loading, and an example proof command. FLUX.1 has package-owned FlowMatch scheduling, transformer, VAE loading/decoding, sampling, and an example proof command over the same snapshot resolver. Z-Image-Turbo, Qwen-Image/Qwen-Image-2512, and FLUX.2 Klein now have package-owned runtime/proof paths with bounded official checkpoint evidence; Qwen-Image Edit/Edit Plus now has manifest recognition, package-owned VAE image encode, multi-reference packed-latent preparation, multi-segment RoPE, and `zero_cond_t` transformer support, while edit denoising concat/slice remains fenced. SD3/3.5 has runtime, loading, prompt conditioning, and a finite proof command foundation, with official gated checkpoint proof blocked on access. LTX-Video and LTX-2 have snapshot recognition, typed component config parsing, latent geometry/packing helpers, video/audio RoPE coordinate helpers, transformer runtime/loading foundations, VAE decode foundations, latent upsampling foundations, and plain LTX-2 vocoder waveform decode; full LTX-2 prompt/audio-video proof assembly, image editing, inpainting, ControlNet, bandwidth extension, LTX-2.3 branches, and broader media families remain Phase 10 work. |
