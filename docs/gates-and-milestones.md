...
| Z-Image-Turbo is the next speed-first runtime target after FLUX.1 | Base `ZImagePipeline` snapshots are recognized, configs parse, the package-owned base runtime covers latent patching, Z RoPE, single-stream transformer denoising, VAE decode layout, and dense weight mapping, and `examples/z-image` owns the finite proof command; official checkpoint evidence remains the next gate |
| Qwen-Image is tracked as image generation, not Qwen VLM serving | `@mlxts/diffusion` recognizes base `QwenImagePipeline` snapshots, parses and loads Qwen-Image transformer plus Qwen-specific VAE configs/weights, owns true-CFG denoising over prepared Qwen2.5-VL text conditioning, and `examples/qwen-image` owns the finite proof command; official `Qwen/Qwen-Image-2512` checkpoint evidence remains the next gate |
