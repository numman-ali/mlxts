...
| Phase 10 proof surfaces | `bun run check:phase10-proofs` | Phase 10 example proof code typechecks; BMP/WAV artifact and report verifier tests pass |
...
| Media generation proof | `bun run examples/image-proof/verify-report.ts <report-json>` or `bun run examples/ltx-video/verify-report.ts <report-json>` against the saved proof JSON | `bun run check:phase10-proofs`; real checkpoint proof rerun when model files and memory budget fit |
...
| Criterion | How to verify |
|-----------|--------------|
| Stable Diffusion / SDXL has a real checkpoint proof | Local or Hub-resolved Diffusers snapshot generates an image artifact through `examples/stable-diffusion/` |
...
| Qwen-Image is tracked as image generation, not Qwen VLM serving | `@mlxts/diffusion` recognizes base `QwenImagePipeline`, `QwenImageEditPipeline`, and `QwenImageEditPlusPipeline` snapshots, parses and loads Qwen-Image transformer plus Qwen-specific VAE configs/weights for the proved text-to-image path, owns true-CFG denoising over prepared Qwen2.5-VL text conditioning, exposes package-owned VAE image encode and multi-reference packed-latent preparation, supports multi-segment RoPE plus `zero_cond_t` transformer modulation, `examples/qwen-image` owns the finite proof command, official `Qwen/Qwen-Image-2512` has bounded checkpoint evidence, and edit runtime remains fenced until processor reference-image conditioning and denoising concat/slice land |
| FLUX.2 Klein is treated as a separate later family | The roadmap names `Flux2KleinPipeline` / `Flux2Transformer2DModel` separately from FLUX.1; the current package path recognizes Klein snapshots, parses configs, loads transformer/VAE weights, samples prepared embeddings with FLUX.2 latent ids, FlowMatch denoising, external CFG, VAE batch-norm decode semantics, exposes `examples/flux2` as the finite proof command, and official `black-forest-labs/FLUX.2-klein-4B` has bounded checkpoint evidence |
| Stable Diffusion 3 / 3.5 is tracked as one MMDiT/flow family | `@mlxts/diffusion` recognizes `StableDiffusion3Pipeline` snapshots, parses `SD3Transformer2DModel` and AutoencoderKL configs including SD3.5 dual-attention metadata, and keeps runtime/weights/proof as explicit follow-up gates |
...
| Media proof artifacts are machine-checkable | Saved image proof JSON verifies through `examples/image-proof/verify-report.ts`; saved LTX proof JSON verifies through `examples/ltx-video/verify-report.ts` |
