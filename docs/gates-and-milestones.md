...
| FLUX.2 Klein is treated as a separate later family | The roadmap names `Flux2KleinPipeline` / `Flux2Transformer2DModel` separately from FLUX.1, the current snapshot/config skeleton recognizes Klein, and the prepared-embedding sampling foundation covers latent ids, FlowMatch denoising, external CFG, and VAE batch-norm decode semantics without claiming transformer/VAE weight loading or proof-command support |
