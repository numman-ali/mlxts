...
| FLUX.2 Klein is treated as a separate later family | The roadmap names `Flux2KleinPipeline` / `Flux2Transformer2DModel` separately from FLUX.1; the current package path recognizes Klein snapshots, parses configs, loads transformer/VAE weights, samples prepared embeddings with FLUX.2 latent ids, FlowMatch denoising, external CFG, VAE batch-norm decode semantics, and exposes `examples/flux2` as the finite proof command while bounded real checkpoint evidence remains pending |
