...
| `@mlxts/transformers` + `@mlxts/serve` | prefix cache | prompt-prefix reuse and cache-hit lifecycle | cache backend | family-owned cache snapshots, serve telemetry | landed first single-request message path; batch/paged LCP reuse pending | phase 9 cache pass | snapshot/fork tests across full KV, layer-pattern, and Qwen hybrid caches + repeated-chat cache-hit tests; live TTFT benchmarks still required |
