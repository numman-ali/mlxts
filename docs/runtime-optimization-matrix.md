...
| `@mlxts/serve` | production metrics | bounded request/generation/scheduler/batch/memory counters and histograms | observability seam | `@mlxts/serve` | implemented first pass; cache-specific metrics deepen with future cache backends | phase 9 | `/metrics` tests + bench report correlation |
