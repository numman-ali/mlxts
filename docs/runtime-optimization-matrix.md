...
| `@mlxts/serve` | serving strategy config | operator flags to typed runtime strategy | strategy seam | `@mlxts/serve` | not yet | next phase 9 tranche | `/info` selected-strategy output + routing tests + rejected-combination tests |
| `@mlxts/serve` | production metrics | request/scheduler/cache/memory counters | observability seam | `@mlxts/serve` | not yet | phase 9 | `/metrics` tests + bench report correlation |
| `@mlxts/transformers` + `@mlxts/serve` | prefix cache | LCP prompt reuse and cache-hit lifecycle | cache backend | shared cache contracts, serve telemetry | not yet | phase 9 cache pass | hit/miss tests + Qwen non-trimmable-state design + long-context bench evidence |
